...
<template>
    <v-card style="border-radius:0px">
      ...
      <v-text-field v-for="(text,i) in this.template.textfield" :key="'text'+i" v-model=dataAnnotations[text] class="mx-2" variant="outlined" density="compact" v-bind:label="text" @change="changeDetect"></v-text-field>
      <v-textarea v-model=dataAnnotations.comment class="mx-2" label="comment" rows="4" prepend-inner-icon="mdi-comment" @change="changeDetect" variant="outlined" density="compact"></v-textarea>
    </v-card>
  </template>
